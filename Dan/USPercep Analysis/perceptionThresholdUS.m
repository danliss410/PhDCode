function perceptionThresholdUS(subject, leg)
...
subjectUS = ['YAUSPercep' subject '_US'];

subjectYesNo = ['YAUSPercep' subject '_YesNo'];

subject1 = ['YAUSPercep' subject];
...
for iSub = 1:size(name,1)
    if leg == 'R'
        ...
        % Grouping the negative velocity values 
        neg_v = vel(vel<0);
        ...
        for iVel = 1:length(pos_v)
            ...
            % Getting negative perturbation indicies 
            ...
            % Calculating the amount of correct negative perturbations 
            perNTrueR{iVel} = sum(perNegR{:,iVel});
            % Calculating the total number of perturbations for each
            % negative velocity 
            perNTotalR{iVel} = length(perNegR{:,iVel});
        end
        ...
        percepNegR(:,:) = [-neg_v'; perNTrueR{:}; perNTotalR{:};]';
        ...
        % perturbation trials
        plotNames = {[name{iSub} ' Pos Dom Leg ']; [name{iSub} ' Neg Dom Leg ']};
        plotData = {percepPosR; percepNegR};
        for iPlot = 1:2
            h = figure; 
        ...
        % perturbation trials
        plotNames = {[name{iSub} ' Pos Dom Leg ']; [name{iSub} ' Neg Dom Leg ']};
        plotData = {percepPosL; percepNegL};
        for iPlot = 1:2
            h = figure; 
            filename = [plotNames{iPlot}  datestr(now, 'mm-dd-yyyy')];
            filenamePDF = strcat(filename, '.pdf'); 
