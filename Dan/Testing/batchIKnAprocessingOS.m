function batchIKnAprocessingOS(strSubject)
% 
...
%   for both the CoM calcualtions and for the MuscleAnalysis. This will do
...
% Analysis models.
if ispc    
    startPath = ['G:\Shared drives\NeuroMobLab\Projects\Perception\Processed Data\Dan\OpenSim\YAPercep Outputs\YAPercep' strSubject '\'];
...
trc_data_folder = ['G:\Shared drives\NeuroMobLab\Projects\Perception\Processed Data\Dan\OpenSim\YAPercep Outputs\YAPercep' strSubject '\MarkerAndGRFDataFiles\'];
...
genericSetupForIK = 'G:\Shared drives\NeuroMobLab\Projects\Perception\Processed Data\Dan\OpenSim\YAPercep Outputs\XML and Model\SetupIK.xml';
...
genericSetupForAnalysis = 'G:\Shared drives\NeuroMobLab\Projects\Perception\Processed Data\Dan\OpenSim\YAPercep Outputs\XML and Model\BKSetup_template.txt';
analysis = AnalyzeTool(genericSetupForAnalysis);
...
for trial= 1:nTrials
    ...
    %BodyKinemtatics tool and adding the analysis to the model configuration
    bkTool = BodyKinematics();
    bkTool.setExpressResultsInLocalFrame(0);
    bkTool.setInDegrees(1);
    model.addAnalysis(bkTool);

    
    %Muscle Analysis tool and adding the analysis to the model configuration
