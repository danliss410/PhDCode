function [prepData, d] = Effect_SizeNacob(strSubject, speedLow, speedHigh)
...
%                 tempStore.(nameP).MoSML.(pertCount{pertCount2}) = nan(2*gcHalf,size(nameFrames,1)); 
            ...
            for i = 1:gcLen
                ...
                tempStore.(nameP).CoMPosx.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Pos{tempTrials}(gcStart+i -1,1);
                tempStore.(nameP).CoMPosy.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Pos{tempTrials}(gcStart+i -1,2);
                tempStore.(nameP).CoMPosz.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Pos{tempTrials}(gcStart+i -1,3);
                tempStore.(nameP).CoMVelx.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Vel{tempTrials}(gcStart+i -1,1);
                tempStore.(nameP).CoMVely.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Vel{tempTrials}(gcStart+i -1,2);
                tempStore.(nameP).CoMVelz.(pertCount{pertCount2})(i,iCycle) = dynamicStabilityTable.CoM_Vel{tempTrials}(gcStart+i -1,3);
...
if ispc
    tabLoc = ['G:\Shared drives\NeuroMobLab\Projects\Perception\Processed Data\Dan\Results\NACOB\'];
...
% subject 
save([tabLoc 'nacobES_' subject1], 'prepData', 'd', '-v7.3');
