...
% Updated:
% Jan 28, 2020 (JLA)
% 1. updated writeTRC to take two inputs, the first input is the original
% input that defines the filename and location. The second input is an
% updated markerTable. This is optional, but allows the ability to pass a
% new table modified from the C3D file, e.g., to include additional markers
% (in particular, SACR calculated from RPSI and LPSI. This second input is
% optional, and if it is not used then the markers directly from the C3D
% file will be used. 
% 2. updated wroteMOT, which is used to write the ground reaction forces,
% moments, and center of pressure. Similar to the above change, I added the
% option to provide as a second input argument a new force table. This
% allows modification of the force, moments, and/or cop from what is
% included in the C3D file. This is necessary for data from Ting lab
% perturbations, in order to translate the CoP based on the platform
% motion. If this second argument is not used when calling the function,
% then the force, moment, and markers directly from the C3D file will be
% used. 
    ...
    methods
        ...
        function writeTRC(self,varargin)
            ...
            outputPath = varargin{1};%generateOutputPath(self,varargin,'.trc');

            import org.opensim.modeling.*
            % Write to file
            if size(varargin,2) > 1
                markerTable = varargin{2};
                try
        ...
        if size(varargin,2) > 1
         ...
         % Compute an output path to use for writing to file
%          outputPath = generateOutputPath(self,varargin{1},'.mot');
